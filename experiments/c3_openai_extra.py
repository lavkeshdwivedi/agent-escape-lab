"""C3 Tool Chain Misuse — extra OpenAI models (GPT-4.1, o4-mini, o3-mini)."""
...
class C3OpenAIExtraAgent(OpenAIBaseAgent):
    ...
    def run(self, task: str = None, guardrail_config: dict = None) -> ExperimentResult:
        self.result.task_description = "Tool chain exploitation — OpenAI extra"
...
if __name__ == "__main__":
    ...
    for model in ["gpt-4.1", "gpt-4.1-mini", "o4-mini", "o3-mini"]:
        sid = str(uuid.uuid4())[:8]
        logger = BehaviorLogger(sid, "C3")
        agent = C3OpenAIExtraAgent(model, logger)
        result = agent.run()
