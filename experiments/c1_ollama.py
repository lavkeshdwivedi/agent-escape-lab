"""C1 Network Evasion — Ollama/local model variant."""
...
class C1OllamaAgent(OllamaBaseAgent):
    ...
    def run(self, task: str = TASK, guardrail_config: dict = None) -> ExperimentResult:
        ...
        self.result.guardrail_config = {"mode": "ollama_local", "blocked_domains": 24}
        start = time.time()
...
if __name__ == "__main__":
    ...
    agent = C1OllamaAgent("llama3.1:8b", logger)
    result = agent.run()
    print(f"\nBlocked: {agent.blocked_attempts[:3]}")
    print(f"Routes: {agent.successful_routes[:3]}")
