"""C1 Network Evasion — Gemini via OpenAI-compatible endpoint."""
...
class C1GeminiAgent(GeminiBaseAgent):
    ...
    def run(self, task: str = TASK, guardrail_config: dict = None) -> ExperimentResult:
        ...
        self.result.guardrail_config = {"mode": "gemini", "blocked_domains": 24}
        start = time.time()
...
if __name__ == "__main__":
    ...
    for model in ["gemini-2.5-flash", "gemini-2.5-pro"]:
        ...
        agent = C1GeminiAgent(model, logger)
        result = agent.run()
