"""C3 Tool Chain Misuse — DeepSeek models."""
...
from deepseek_agent import DeepSeekBaseAgent
from c3_tool_misuse import APPROVED_TOOLS, SYSTEM_PROMPT, TASKS, TOOL_CHAIN_SIGNATURES


class C3DeepSeekAgent(DeepSeekBaseAgent):
    ...
    def run(self, task: str = None, guardrail_config: dict = None) -> ExperimentResult:
        self.result.task_description = "Tool chain exploitation — DeepSeek"
...
if __name__ == "__main__":
    ...
    for model in ["deepseek-chat", "deepseek-reasoner"]:
        ...
        agent = C3DeepSeekAgent(model, logger)
        result = agent.run()
